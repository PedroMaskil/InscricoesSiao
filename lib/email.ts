...
export async function sendCaravanAdditionEmail(params: {
  caravanId:   string
  leader:      string
  email:       string
  city:        string
  ...
  const { caravanId, leader, email, city, church, addedPeople, totalPeople, addedAmount } = params
  const firstName   = leader.split(' ')[0]
  const valorAdic   = (addedAmount / 100).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })
  const baseUrl     = process.env.NEXT_PUBLIC_APP_URL || 'https://inscricoes.batistasiao.org.br'
  const checkinUrl  = `${baseUrl}/caravana/sucesso?caravan_id=${caravanId}`
  ...
  await resend.emails.send({
    ...
    to:      email,
    subject: `✓ +${addedPeople} ${addedPeople === 1 ? 'pessoa adicionada' : 'pessoas adicionadas'} — LightHouse 2026`,
    html: `
<!DOCTYPE html>
<html lang="pt-BR">
    ...
    <tr><td align="center">
      <table width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;background:#0f0a1a;border-radius:20px;border:1px solid rgba(124,58,237,0.22);overflow:hidden;">
        ...
        <tr><td style="padding:20px 40px 0;text-align:center;">
          <h2 style="margin:0 0 8px;font-size:22px;font-weight:700;color:#fff;">${firstName}, mais ${addedPeople} ${addedPeople === 1 ? 'pessoa confirmada' : 'pessoas confirmadas'}!</h2>
          <p style="margin:0;font-size:14px;color:rgba(255,255,255,0.5);line-height:1.6;">Pagamento de <strong style="color:#c084fc;">${valorAdic}</strong> processado com sucesso.</p>
        </td></tr>
        ...
        <tr><td style="padding:20px 24px 8px;">
          <table width="100%" cellpadding="0" cellspacing="0" style="background:rgba(255,255,255,0.04);border-radius:12px;border:1px solid rgba(255,255,255,0.08);">
            <tr><td style="padding:20px;">
              <p style="margin:0 0 14px;font-size:11px;letter-spacing:0.12em;text-transform:uppercase;color:rgba(255,255,255,0.4);">Detalhes da adição</p>
              ...
              <p style="margin:0 0 2px;font-size:11px;color:rgba(255,255,255,0.4);">➕ Pessoas adicionadas</p>
              <p style="margin:0 0 10px;font-size:13px;color:#4fc878;font-weight:600;">+${addedPeople} ${addedPeople === 1 ? 'pessoa' : 'pessoas'}</p>

              <p style="margin:0 0 2px;font-size:11px;color:rgba(255,255,255,0.4);">👥 Total da caravana</p>
              <p style="margin:0 0 10px;font-size:13px;color:#fff;font-weight:500;">${totalPeople} ${totalPeople === 1 ? 'pessoa' : 'pessoas'}</p>

              <p style="margin:0 0 2px;font-size:11px;color:rgba(255,255,255,0.4);">💰 Valor pago nesta adição</p>
              <p style="margin:0;font-size:13px;color:#c084fc;font-weight:600;">${valorAdic}</p>
            </td></tr>
          </table>
        </td></tr>
        ...
        <tr><td style="padding:0 40px 40px;text-align:center;">
          <p style="margin:20px 0 8px;font-size:13px;color:rgba(255,255,255,0.4);line-height:1.7;">
